		...
		const unsigned* second_word, unsigned second_length, unsigned max_distance)
{
    ...
    else if (second_length == 0) return first_length;
    ...
    if (second_length - first_length > max_distance) return second_length - first_length;
	...
	for (unsigned j = from; j <= to; ++j) {
	    ...
	    const double replace_value = previous_row[j - 1] +
		    ((first_ch != second_ch) ? get_replace_cost(first_index, first_length, first_ch, second_ch) : 0);
		...
		value = min(value, transposition_row[j - 2] +
		            get_transposition_cost(first_index, first_length, first_ch, second_ch));
