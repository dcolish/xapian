...
void BrassSpellingTableNGram::toggle_fragment(fragment frag, const string & word)
{
...
void BrassSpellingTableNGram::populate_word(const string& word, unsigned, vector<TermList*>& result)
{
	string data;
	fragment buf;
...
BrassSpellingTermListNGram::next()
{
	...
	size_t add;
	if (p == data.size() || (add = byte(data[p]) ^ MAGIC_XOR_VALUE) >= data.size() - p)
		throw Xapian::DatabaseCorruptError("Bad spelling termlist");
